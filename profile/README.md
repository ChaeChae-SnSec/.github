...
* **DNS 캐시 우회 취약점**: 공유 DNS 환경에서 타 사용자의 접속 이력(Cache)이 남아있을 경우, 보안 모듈이 실행되지 않고 악성 사이트에 접속되는 보안 홀이 발생합니다.
...
* **Real-time AI Inference**: Unbound Python 모듈을 통해 질의된 도메인의 악성 확률을 0.15초 이내에 추론.
* **Shared Cache Lockdown**: 캐시 우회 차단 및 사용자별 독립적 보안 정책 적용.
* **Dual-Stack NXDOMAIN**: IPv4(A)와 IPv6(AAAA) 레코드를 동시 처리하여 완벽한 접속 차단 구현.
* **Interactive Block Page**: 차단된 사이트의 위험도(Probability) 시각화 및 사용자 '임시 허용' 요청 기능.
...
* **Training Strategy**:
    ...
    2.  **Multi-modal Fusion**: 사전 학습된 두 인코더의 출력 벡터를 결합(Concatenation)하는 계층을 구성합니다.
...
| :---: | :---: |
| ![Extension](./images/extension.png) | ![Dashboard](./images/dashboard.png) |